 ...
 *        sorting them in alphabetical order by author's name. 
...
/**
 * @struct Book
 * @brief This struct includes data: <Author>, <Name>, <Year>, <Notes>
 *
 * Copyright 2012 by Anatoliy Dobrosynets
 */
struct Book
...
void Input(Book* m, int n)
{
	for(int i = 0; i < n; i++)
	{
		...
		cout << "Enter the notes of the " << i + 1 << " book: ";
		gets(m[i].no);
	}
}
...
void Output(Book* m, int n)
{
	...
	for(int i = 0; i < n; i++)
	{
		cout << m[i].a << "  |  " << m[i].n << "  |  " << m[i].y << "  |  " << m[i].no << endl;
	}
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	cout << "\nList of books published before 1970:\n";
	cout << "\n<Author>  |  <Name>  |  <Year>  |  <Notes>\n";
